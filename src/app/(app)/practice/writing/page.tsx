    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            if (!sessionSaved) {
                ...
                if (user) await supabase.from('sessions').insert({ user_id: user.id, type: 'writing', score: avg, duration: 10 })
    ...
    return (
        <div className="min-h-screen bg-[#080810] text-white">
            <div className="max-w-2xl mx-auto px-4 py-10">
                ...
                <h1 className="text-2xl font-bold mb-1">✍️ Writing Practice</h1>
                <p className="text-zinc-400 text-sm mb-6">AI-powered feedback on your writing</p>
                ...
                {!result ? (
                    ...
                    >
                        {grading ? <><span className="animate-spin">⟳</span> Grading...</> : 'Submit for Review →'}
                    </button>
                ) : (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                        {/* Score Bars */}
                        <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6">
                            <h3 className="font-semibold mb-5">Your Scores</h3>
                        ...
                        {result.mistakes.length > 0 && (
                            <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6">
                                <h3 className="font-semibold mb-4">Grammar Corrections</h3>
