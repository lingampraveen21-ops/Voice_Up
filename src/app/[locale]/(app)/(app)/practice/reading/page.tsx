...
export default function ReadingPage() {
    ...
    const t = useTranslations("Practice.reading")
    const openFeedback = useFeedbackStore(state => state.openFeedback)
    const { isSupported, startRecording, stopRecording, isRecording } = useVoiceRecorder()
    ...
    const handleWordClick = async (word: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            setPopup(p => p ? { ...p, loading: false, definition: t("defNotAvailable") } : null)
        }
    }
    ...
    const checkBlanks = async () => {
        ...
        toast[score >= 70 ? 'success' : 'error'](t("score", { score }))
        ...
        <div className="min-h-screen bg-[#080810] text-white" onClick={() => setPopup(null)}>
            ...
            <div className="max-w-2xl mx-auto px-4 py-12" ref={articleRef}>
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-xs text-zinc-500">{t("minRead", { min: ARTICLE.readTime })}</span>
                </div>
                <h1 className="text-2xl font-bold mb-8">{ARTICLE.title}</h1>

                <p className="text-xs text-zinc-500 mb-4">{t("instruction")}</p>
                ...
                <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6 mb-6">
                    <h3 className="font-semibold mb-4">{t("fillBlanks")}</h3>
                ...
                <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6">
                    <h3 className="font-semibold mb-1">{t("voiceChallenge")}</h3>
                    <p className="text-xs text-zinc-400 mb-4">{t("voiceDesc")}</p>
                    <button
                    ...
                    >
                        ...
                        {isListening ? t("stopRecording") : t("startSummary")}
                    </button>
                </div>
            </div>
