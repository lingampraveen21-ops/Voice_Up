...
export const XPCard: FC<XPCardProps> = ({ xp }) => {
    ...
    if (xp > 500) rank = t("rankLearner")
    if (xp > 2000) rank = t("rankSpeaker")
    if (xp > 5000) rank = t("rankFluent")
    if (xp > 10000) rank = t("rankMaster")
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start justify-between z-10">
                <div>
                    <p className="text-zinc-400 font-medium text-sm mb-1 uppercase tracking-wider">{t("totalExperience")}</p>
                    <div className="flex items-baseline gap-2">
                        <h2 className="text-4xl font-black font-heading text-white drop-shadow-lg">
                            <CountUp end={xp} duration={2} />
                        </h2>
                        <span className="text-zinc-400 font-medium tracking-wide">{t("xpLabel")}</span>
                    </div>
                </div>
            ...
            <div className="mt-6 z-10 flex items-center justify-between">
                <div className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-sm font-medium text-zinc-300">
                    {t("rank")}: <span className="text-white">{rank}</span>
                </div>
                <p className="text-sm font-bold text-emerald-400">{t("xpToday", { amount: 150 })}</p>
            </div>
